...
# Probamos imputadores distintos
from sklearn.preprocessing import LabelEncoder
from sklearn.experimental import enable_iterative_imputer
from sklearn.impute import IterativeImputer, KNNImputer
from sklearn.ensemble import RandomForestRegressor, RandomForestClassifier

variables_numericas = ['h1n1_concern', 'h1n1_knowledge',
       'behavioral_antiviral_meds', 'behavioral_avoidance',
       'behavioral_face_mask', 'behavioral_wash_hands',
       'behavioral_large_gatherings', 'behavioral_outside_home',
       'behavioral_touch_face', 'doctor_recc_h1n1', 'doctor_recc_seasonal',
...
data_x_tmp[variables_categoricas] = data_x_tmp[variables_categoricas].apply(lambda series: pd.Series(
...
print("deifno imputer 1")
imp_cat = KNNImputer(n_neighbors=1)

print("deifno imputer 2")
data_x_tmp[variables_numericas] = imp_num.fit_transform(data_x_tmp[variables_numericas])
print("Aplico imputer")
data_x_tmp[variables_categoricas] = imp_cat.fit_transform(data_x_tmp[variables_categoricas])
print("Aplico imputer 2")
...
data_x_tmp = data_x_tst
data_x_tmp[variables_categoricas] = data_x_tmp[variables_categoricas].apply(lambda series: pd.Series(
    LabelEncoder().fit_transform(series[series.notnull()]),
    index=series[series.notnull()].index
))
data_x_tmp[variables_numericas] = imp_num.transform(data_x_tmp[variables_numericas])
print("Aplico imputer")
data_x_tmp[variables_categoricas] = imp_cat.transform(data_x_tmp[variables_categoricas])
print("Aplico imputer 2")

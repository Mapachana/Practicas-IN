...
mask =  data_x["employment_status"].isna() 
data_x_tmp.loc[mask, "employment_status"] = "Employed" 
...
most_frequent = data_x_tmp["employment_industry"].value_counts().index
selected = 0
while most_frequent[selected] == no_procede or most_frequent[selected] == no_empleado or most_frequent[selected] == empleado_healthcare_industry:
    selected += 1
    
data_x_tmp["employment_industry"].fillna(most_frequent[selected])
"""
...
data_x_tmp.drop(columns=['hhs_geo_region', 'census_msa', 'race', 'sex', 'marital_status'], inplace=True)
X.drop(columns=['hhs_geo_region', 'census_msa', 'race', 'sex', 'marital_status'], inplace=True)
